...
``varco_sa``'s ``WebhookSubscription`` repository.
...
Nothing under ``varco_sa`` implements this repository yet — the import below
is expected to fail with ``ModuleNotFoundError``/``ImportError``.
"""
...
class TestSAWebhookSubscriptionRepositoryConformance(WebhookSubscriptionRepositoryConformance):
    ...
    @pytest.fixture
    async def repo(self, postgres_url: str):
...
@pytest.fixture
async def repo(postgres_url: str):
    from varco_sa.webhook import SAWebhookSubscriptionRepository
