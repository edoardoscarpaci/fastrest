...
See the `CLAUDE.md` "Multitenancy" section's Common Pitfalls rows for the
full table (raw `text()` SQL not translated, `TENANT`/`GLOBAL` cache key
mistakes, `init_beanie` global rebind, fan-out misconfiguration,
`mount_tenant_admin` misuse, and more) — every row there has a
corresponding test in Plan 007's phases.
