...
router.py
=========
FastAPI router for the Kafka order-events example.
...
def build_router(
    ...
    """
    Build and return the FastAPI router for the Kafka order-events example.
    ...
    Edge cases:
        - ``producer._produce()`` propagates Kafka errors directly to the
    ...
    async def place_order(body: PlaceOrderRequest) -> dict:
        """
        Publish an ``OrderPlacedEvent`` to the ``"orders"`` Kafka topic.

        Returns HTTP 202 Accepted — the event is handed off to the bus
        asynchronously; delivery to handlers may lag slightly (Kafka consumer
        group coordination adds a small overhead vs Redis Pub/Sub).
        ...
        Raises:
            HTTPException 500: If the Kafka publish call fails.
        """
    ...
    async def list_notifications() -> list[NotificationItem]:
        ...
        Edge cases:
            ...
            - Kafka AT_LEAST_ONCE delivery may cause duplicates in this list
