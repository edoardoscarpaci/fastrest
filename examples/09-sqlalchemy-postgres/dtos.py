...
DESIGN: separate Create/Read/Update DTOs over a single combined model
    ...
    ✅ Validation (field lengths, required vs optional) is enforced by Pydantic
       per operation type.
...
class PostUpdate(UpdateDTO):
    ...
    Edge cases:
        - ``author`` can be changed on UPDATE to support ghost-writing or
          corrections.  Stricter access control (ownership checks) would be
          added at the service layer via ``AbstractAuthorizer``.
    """
