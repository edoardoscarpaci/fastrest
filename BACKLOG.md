...
|----|---------|----------|------------|----------|
| RT2-B | **NATS `AT_LEAST_ONCE` does not redeliver after a handler raises** — `varco_nats/varco_nats/bus.py:565-573` acks in a `finally`, and the module's own docstring says the message is acked "whether or not a handler raised … JetStream only redelivers on a process crash." `xfail(strict=True)`, not fixed (never a production patch for a red integration test) | 🔴 must | S | `varco_nats/tests/test_nats_semantics_integration.py::test_at_least_once_redelivers_after_handler_raises` |
| RT2-C | **NATS `ChannelManager.channel_exists()`/`list_channels()` implement a "has messages" predicate, not an "exists" predicate** — `channel.py:377-395`/`:417`: `declare_channel()`'s `channel` argument is documented as "only used for logging", and `list_channels()` returns "channels that currently carry messages". `declare_channel()` → `channel_exists()` is `False` on an empty stream, so 3 of 4 round-trips in the ABC-contract test fail structurally. `xfail(strict=True)` on the three affected assertions, not fixed — this is an ABC-contract gap on the NATS backend, not a test bug | 🔴 must | M | `varco_nats/tests/test_nats_channel_integration.py` |
| RT7a-redis-claim-guard | **`RedisJobStore.reap_expired_leases()` does not release the SET-NX-EX claim guard key `try_claim()` created for the original claim** (`job_store.py:595-605`) — the guard's TTL (`claim_ttl`, default 30s) is independent of `lease_ttl` and is never cleared on reap, so a legitimate re-claim by a second worker is refused with "claim key already held" for up to `claim_ttl` seconds after a correct reap, even though the job is properly `PENDING` again with an advanced `lease_epoch`. Confirmed by lowering `claim_ttl` in an ad-hoc probe (passes at `claim_ttl=1`, fails at the default 30). `varco_sa`'s `SAJobStore` has no equivalent second guard key and does not exhibit this — exactly the cross-backend disagreement `test_redis_job_lease_crash.py`'s own docstring names as "the finding worth having". `xfail(strict=True)`, not fixed | 🔴 must | S | `varco_redis/tests/test_redis_job_lease_crash.py::test_reaped_lease_fences_the_zombie_worker_on_save` (the negative-case sibling test, `test_renewed_lease_keeps_a_second_worker_locked_out`, is unaffected and stays green) |
| RT7b-port-remap | **`docker-py`'s `Container.restart()` did not preserve the host port mapping** in this session's Docker 27.5.1 / WSL2 environment — verified independently of testcontainers/`ChaosContainer` with a raw `docker.from_env()` script: a container published on an ephemeral host port came back on a *different* ephemeral port after `.restart(timeout=5)`. This directly contradicts research 002 §1's port-survivorship claim that Plan 018's whole `ChaosContainer.restart()` design (and every restart-based chaos test's "capture the DSN once, reuse across restarts" pattern) is built on. `testkit/varco_chaos/containers.py`'s `wait_ready()` was hardened in the same session to only match readiness against **new** log output emitted after the restart (the original cumulative-log-history predicate matched instantly and falsely, since a historical "ready" line from the *original* boot is still present in `docker logs` output) — that fix is real and independent of the port issue. The port-remap itself was **not** worked around, because doing so would require rewriting the already-written Phase 6 chaos test modules' DSN-caching pattern, out of this session's scope. Not yet verified against a native Linux dockerd (the actual GitHub Actions runner) — if the nightly `chaos` job is flaky specifically on the restart-based tests (`test_kafka_chaos.py`, `test_sa_chaos.py`, `test_migration_chaos.py`'s fallback path) and not the pause-based ones (`test_redis_chaos.py`, `test_nats_health_chaos.py`, which were verified green), this is the first suspect | 🔴 must | M | Ad-hoc probe (raw `docker-py`, no testcontainers): `before restart ports: {'5432/tcp': [{'HostPort': '32811'}]}` → `after restart ports: {'5432/tcp': [{'HostPort': '32812'}]}`; reproduced identically through `ChaosContainer`/`test_sa_chaos.py` (`ConnectionRefusedError` connecting via the pre-restart-captured DSN, while a fresh `get_exposed_port()` query after restart succeeds) |
| WD-1 | **WS backpressure margin is machine-dependent and thin** — `test_ws_backpressure_integration.py` required raising the volume to `_N=6000` × 64 KiB payloads (~384 MB in flight) to make the "slow client received strictly fewer than N" assertion engage; smaller volumes (2000×16 KiB, 3000×64 KiB) passed vacuously. Root cause: uvicorn's `websockets_impl` buffers server-side writes with no `write_limit` applied (§RT4-backpressure explicitly declined to set one), so a slow client's `max_queue=1` does not propagate into varco's per-client `asyncio.Queue` until the OS/uvicorn buffer itself saturates. If this ever flakes, the module's own Edge-cases table governs: raise the volume, never relax the assertion | 🟡 should | S | `varco_ws/tests/test_ws_backpressure_integration.py` module docstring's calibration table; 3/3 consecutive clean local runs (~95s each) |
| RT7-toxiproxy | **Toxiproxy (graded latency, bandwidth throttling, one-directional faults) deferred to 3.1** — `docker pause`/`restart` cover every 3.0.0 chaos assertion (a genuine black hole or a genuine restart), but cannot express latency injection or partial/asymmetric failure. Plan 018 §RT7-toxiproxy declined to adopt it for 3.0.0 | 🟢 nice | M | `plans/018-reliability-floor-rt-integration-and-chaos.md` §RT7-toxiproxy (four ❌s recorded there) |
| RT9-beanie-migrations | **Beanie/Mongo migration integration coverage** — Plan 018 §RT9-scope's residual: `AlembicMigrator`'s SQL-side lock/lifecycle paths now have real-Postgres coverage (RT9 above), but `varco_beanie.migration.BeanieMigrator`'s index-mode lifecycle has none. Not attempted this plan — Mongo has no advisory-lock primitive equivalent to Postgres's, so the lock-timeout scenario would need a different mechanism entirely, not a port of the SQL tests | 🟡 should | M | `varco_beanie/varco_beanie/migration.py` (`BeanieMigrator`); plan 018 §RT9-scope |
| RT4-ws-scale | **Many-connection WS/SSE scale test** — blocked on undocumented GitHub Actions file-descriptor limits for a test that would open hundreds/thousands of concurrent WebSocket connections. §RT4-backpressure scoped Plan 018 to per-client isolation (2 connections) rather than fleet-scale connection counts | 🟢 nice | M | `plans/018-reliability-floor-rt-integration-and-chaos.md` §RT4-backpressure |
